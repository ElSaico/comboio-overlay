const chars = [ // source: https://github.com/dhepper/font8x8
...
function draw(el, text, length, ledWidth, ledHeight, interval) {
    ...
    for (let i = 0; i < length*8; ++i) {
        leds.push([]);
        for (let j = 0; j < 8; ++j) {
            leds[i].push(svg.ellipse(ledWidth, ledHeight).fill(ledOff).move(ledWidth*i, ledHeight*j));
        }
    }
    ...
    function drawStep() {
        leds.map((row, i) => {
            ...
            row.map((led, j) => {
                if ((textArray[j][Math.floor(trueI/8)] >> trueI%8) & 1) {
                    led.fill(ledOn);
                } else {
                    led.fill(ledOff);
                }
    ...
    if (text.length > length) {
        setInterval(drawStep, interval);
    } else {
        offset = (text.length - length) * 4;
        drawStep();
    }
}
